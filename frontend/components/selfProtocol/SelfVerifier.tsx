...
function SelfVerifier() {
	...
	const saveOptionsToServer = async () => {
		...
		try {
		  ...
		  const res = await fetch('/api/saveOptions', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ userId, options }),
		  });
	  
		  const text = await res.text(); // JSONかもしれないし、HTML（エラー）かもしれない
		  console.log('API raw response:', text); // ← エラーページがHTMLならわかる
	  
		  if (!res.ok) {
			let errorMessage = 'Failed to save options';
			try {
			  const errorJson = JSON.parse(text); // JSON形式ならパースして使う
